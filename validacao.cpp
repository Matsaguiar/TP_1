...
int validaCidade(string est){
    int val = 0, i, letr = 0;
    for (i = 0; est[i] != '\0'; i++){
        if (maiuscula(est[i]) || minuscula(est[i]))
            letr++;
        if (est[i] == ' ' && est[i+1] == ' ')
            return 0;

        if (!maiuscula(est[i]) && !minuscula(est[i]) && est[i] != ' ' && !numero(est[i]) && est[i] != '.')
            return 0;
...
int validaEstado(string estado){
    int i;
    if (estado.size() != 2)
        return 0;
    for (i = 0; i < 2; i++){
        if (!maiuscula(estado[i]))
            return 0;
    }
    return 1;
}
